...
int sys_close(int fd) {
  struct fs_fd *fsfd = fd_get(fd);
  if (fsfd == NULL) return -STATUS_EINVAL;
...
int sys_read(int fd, void *buf, size_t len) {
  struct fs_fd *fsfd = fd_get(fd);
  if (fsfd == NULL) return -STATUS_EBADF;
  if (buf == NULL || (int32_t)len < 0) return -STATUS_EINVAL;
...
/* Note: Check the whence parameter and calcuate the new offset value before do
 * file_seek() */
...
int sys_unlink(const char *pathname) {
  ...
  if (res == -FR_NO_FILE) return -STATUS_ENOENT;
  return res;
}

int sys_ls(const char *pathname) {
  int res = file_ls(pathname);
  if (res == -FR_NO_PATH) return -STATUS_ENOENT;
  return res;
}
