 ...
 */
// TODO: Lab6, add spinlock, modify ts->parent_id
...
int task_create()
{
	...
	for (va = USTACKTOP - USR_STACK_SIZE; va < USTACKTOP; va += PGSIZE) {
		...
		if (page_insert(ts->pgdir, pp, (void *)va, PTE_W | PTE_U) == -1) {
			return -1;
		}
	}
	...
	ts->parent_id = thiscpu->cpu_task ? thiscpu->cpu_task->task_id : pid;
...
void sys_kill(int pid)
{
	if (pid > 0 && pid < NR_TASKS)
	{
		...
		spin_unlock(&task_lock);
		int i, j, found = 0;
		for (i = 0; i < ncpu && !found; ++i) {
			for (j = 0; j < cpus[i].cpu_rq.cnt; ++j) {
				if (pid == cpus[i].cpu_rq.rq[j]->task_id) {
					found = 1;
				}
				if (found && j != (cpus[i].cpu_rq.cnt - 1)) {
					cpus[i].cpu_rq.rq[j] = cpus[i].cpu_rq.rq[j + 1];
				}
			}
		}
		cpus[i].cpu_rq.rq[cpus[i].cpu_rq.cnt - 1] = NULL;
		cpus[i].cpu_rq.cnt--;
		sched_yield();
	}
}
 ...
 *   1) cur_task to thiscpu->cpu_task
 *   2) copy user stack
 *   3) dispatch to different cpu
 */
int sys_fork()
{
	...
	if ((uint32_t)thiscpu->cpu_task)
	{
		...
		// int va;
		// for (va = USTACKTOP - USR_STACK_SIZE; va < USTACKTOP; va += PGSIZE) {
		// 	pte_t *pte_src = pgdir_walk(cur_task->pgdir, va, 0);
		// 	pte_t *pte_dst = pgdir_walk(tasks[pid].pgdir, va, 0);
...
//
// summary:
//   1) tss to thiscpu->cpu_tss
//   2) cpu_task to thiscpu->cpu_task
//   3) modify ts_esp0
//   4) GD_TSS0 to GD_TSS0 + thiscpu->cpu_id * 8
//   5) idle ap
//   6) init Runqueue
