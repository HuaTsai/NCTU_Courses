...
page_init(void)
{
	...
	// free pages!
	// TODO: Lab5, maintain num_free_pages
	/* TODO: Lab6, avoid MPENTRY_ADDR
	...
	for (i = 0; i < npages; ++i) {
		...
		} else if (i < npages_basemem && i == PGNUM(MPENTRY_PADDR)) {
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
...
{
	// TODO: Lab4, page_alloc
	// TODO: Lab5, maintain num_free_pages
	// TODO: Lab6, add spin lock
	spin_lock(&page_lock);
	if (!page_free_list) {
 ...
 */
/* TODO: Lab6, cpu kernel stack and mmio
 * You should also map:
...
setupkvm()
{
	...
	if (pp) {
		...
		for (i = 0; i < NCPU; ++i) {
			uintptr_t kstacktop_i = KSTACKTOP - i * (KSTKSIZE + KSTKGAP);
			boot_map_region(pgdir, kstacktop_i - KSTKSIZE, ROUNDUP(KSTKSIZE,PGSIZE), PADDR(&percpu_kstacks[i]), PTE_W);
		}
		extern uint32_t *lapic;
		boot_map_region(pgdir, (uintptr_t)lapic, PGSIZE, lapicaddr, PTE_PCD | PTE_PWT | PTE_W);
	}
	return pgdir;
}


/* TODO: Lab5, sys_get_num_free_page
