...
/* function implementations in homework 3 */
...
int sigaction(int signum, const struct sigaction *act,
  ...
  if (act) {
    ...
    kact.sa_mask.sig = act->sa_mask.sig;
    kact.sa_flags = act->sa_flags | SA_RESTORER;
    kact.sa_restorer = restorer;
  }
  long ret = sys_rt_sigaction(signum, act ? &kact : NULL, oact ? &koact : NULL,
                              sizeof(sigset_t));
  if (oact && ret >= 0) {
    oact->sa_handler = koact.sa_handler;
    oact->sa_mask.sig = koact.sa_mask.sig;
