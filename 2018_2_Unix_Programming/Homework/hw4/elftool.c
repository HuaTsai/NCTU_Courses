...
int elf_close(elf_handle_t *e) {
  if (e == NULL) return 0;
  if (e->dsym) {
    free(e->dsym->symbol);  // FIXME: add free
    free(e->dsym);
    e->dsym = NULL;
  }
  if (e->sym) {
    free(e->sym->symbol);  // FIXME: add free
    free(e->sym);
    e->sym = NULL;
  }
  if (e->strtab) {
    elf_free_strtab(e->strtab);
    e->strtab = NULL;
  }
  if (e->shdr) {
    free(e->shdr);
...
#define ITER_PHDR(x)                             \
  do {                                           \
    Elf##x##_Phdr *p = elf##x##_getphdr(e->elf); \
    for (i = 0; i < e->phnum; i++) {             \
...
#define ASSIGN_SHDR(x)                        \
  do {                                        \
    Elf##x##_Shdr *s = elf##x##_getshdr(scn); \
...
elf_strtab_t *elf_load_strtab(elf_handle_t *e) {
  ...
  for (i = 0; i < e->shnum; i++) {
    ...
    if ((curr->data = (char *)malloc(e->shdr[i].size)) == NULL) goto err_quit;
    if (pread(e->fd, curr->data, e->shdr[i].size, e->shdr[i].offset) !=
        e->shdr[i].size)
      goto err_quit;
    curr->next = strtab;
    strtab = curr;
...
static elf_symtab_t *elf_load_symtab_internal(elf_handle_t *e,
                                              elf_shdr_t *shdr) {
  int i, realerror;
  ...
  if ((symtab = (elf_symtab_t *)malloc(sizeof(elf_symtab_t))) == NULL)
    return NULL;
  if (e->clazz != ELFCLASS32 && e->clazz != ELFCLASS64) {
    errno = EINVAL;
    return NULL;
  }
  ...
  symtab->count = (e->clazz == ELFCLASS32) ? (shdr->size / sizeof(Elf32_Sym))
                                           : (shdr->size / sizeof(Elf64_Sym));
  if ((symtab->symbol = (elf_symbol_t *)malloc(
           sizeof(elf_symbol_t) * symtab->count + shdr->size)) == NULL)
    goto err_quit;
  if (pread(e->fd,
            ((unsigned char *)symtab->symbol) +
                (sizeof(elf_symbol_t) * symtab->count),
            shdr->size, shdr->offset) != shdr->size)
    goto err_quit;

#define ASSIGN_SYM(x)                                                     \
  do {                                                                    \
    Elf##x##_Sym *s =                                                     \
        (Elf##x##_Sym *)(((unsigned char *)symtab->symbol) +              \
                         (sizeof(elf_symbol_t) * symtab->count));         \
